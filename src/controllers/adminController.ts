...
const deleteOneProductController = asyncHandler(
 async (req: Request, res: Response) => {
  const { id } = req.params;
  if (!id) {
...
const deleteallProductsController = asyncHandler(
 async (req: Request, res: Response) => {
  ...
  res
   .status(200)
   .json(
    new ApiResponse(200, allProductDelete, "All Products Delete SuccessFully")
   );
 }
);
